...
**推荐：双击项目根目录的 `start-dev.bat`。** 它会打开前后端两个窗口，等待服务就绪后打开浏览器。运行期间保持两个窗口打开；关闭时分别按 Ctrl+C。端口已被占用时会提示退出，避免重复启动。脚本从自身所在目录运行，不依赖双击时的工作目录。

该脚本只负责启动，首次使用仍需准备独立 Python 环境、前端依赖和 `.env`（这些不会随 Git 上传）。运行 `cmd /c start-dev.bat --check` 可检查启动文件和 Bun 是否齐全。
